...
`skills/agent-slack` to `shhac/agent-skills`. **No manual build, and no manual
formula bump.**
...
5. Verify CI and the outputs:
   ```bash
   gh run watch --repo shhac/agent-slack          # release + Publish skill runs green
...
repo-specific "agent-slack release automation (env-scoped)" one). The skill
...
```bash
ssh-keygen -t ed25519 -N "" -C "agent-slack release automation" -f tap_key
gh repo deploy-key add tap_key.pub -R shhac/homebrew-tap --allow-write \
  --title "agent-slack release automation (env-scoped)"
gh secret set TAP_DEPLOY_KEY --repo shhac/agent-slack --env homebrew-tap < tap_key
